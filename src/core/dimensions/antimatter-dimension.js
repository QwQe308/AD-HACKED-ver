...
export function buyOneDimension(tier) {
  ...
  dimension.currencyAmount = dimension.currencyAmount.add(cost);
...
export function buyMaxDimension(tier, bulk = Infinity) {
  ...
  if (NormalChallenge(9).isRunning || InfinityChallenge(5).isRunning) {
    while (dimension.isAffordableUntil10 && dimension.cost.lt(goal) && bulkLeft > 0) {
      ...
      dimension.currencyAmount = dimension.currencyAmount.add(dimension.costUntil10);
      buyUntilTen(tier);
      bulkLeft--;
    }
    return;
  ...
  dimension.currencyAmount = dimension.currencyAmount.add(Decimal.pow10(maxBought.logPrice));
}
